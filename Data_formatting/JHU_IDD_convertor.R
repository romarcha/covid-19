JHU_IDD_convertor = function(raw_file,col_names){
  ...
  if(nrow(raw_data_point)==0){
    percent_numbers=length(unique(raw_data_quantile$quantile))
    target_data_quantile=as_tibble(matrix(NA,nrow(raw_data_quantile)/percent_numbers,length(col_names))) # target data for point estimate
    colnames(target_data_quantile)=col_names
    for(i in 1:nrow(target_data_quantile)){
      target_data_quantile$forecast_date[i] = (raw_data_quantile$forecast_date)[i]
      target_data_quantile$model_name[i] = strsplit(raw_file,split="/")[[1]][3]
      target_data_quantile$target_date[i] = unique(raw_data_quantile$target_end_date)[i]
      target_data_quantile$lookahead[i] = as.Date(target_data_quantile$target_date[i]) - as.Date(target_data_quantile$forecast_date[i])
      target_data_quantile$location_long[i] = raw_data_quantile$location[i]
      target_data_quantile$prediction_type[i] = raw_data_quantile$type[i]
      
      target_data_quantile$gt_source[i]="gt_jhu"
    ...
    for(i in 1:nrow(target_data_quantile)){
      tem=filter(raw_data_quantile,target_end_date==target_data_quantile$target_date[i],location==target_data_quantile$location_long[i])
    ...
    target_data=target_data_quantile
  }else{
  ...
  for(i in 1:nrow(target_data)){ # fill in the ground truth data
    ...
    target_data$gt_jhu[i]= 
      sum(filter(jhu_truth,Country_Region==target_data$location_long[i])[,target_data$target_date[i]])
    } 
  }
  return(target_data)
}

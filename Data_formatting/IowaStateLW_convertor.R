IowaStateLW_convertor = function(raw_file,col_names){
  ...
  ind.num1=which(col_names=="perc_0.025")
  ind.num2=which(col_names=="perc_0.975")
  target_data_quantile[,c(ind.num1,ind.num2)]=0
  for(i in 1:nrow(target_data_quantile)){
    ...
    if(nrow(tem)>2){
      target_data_quantile[i,c(ind.num1,ind.num2)]=as_tibble(matrix(unique(tem$value),1))
    } else{
      tem=tem[order(tem$value),]
      target_data_quantile[i,c(ind.num1,ind.num2)]=as_tibble(matrix(tem$value,1))
      }
